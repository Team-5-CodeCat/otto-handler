...
if [ ! -f .env.dev ]; then
  if [ -f .env.example ]; then
    ...
    sed -i 's|DATABASE_URL=postgresql://postgres:password@localhost:5432/otto_handler|DATABASE_URL=postgresql://postgres:postgres@db:5432/otto-handler|' .env.dev
    sed -i 's|REDIS_URL=redis://localhost:6379|REDIS_URL=redis://redis:6379|' .env.dev
...
# 2. 시크릿 디렉토리 생성
echo -e "${YELLOW}🔐 시크릿 디렉토리를 설정합니다...${NC}"
if [ ! -d "/workspace/otto-handler/secrets" ]; then
  mkdir -p /workspace/otto-handler/secrets
  echo -e "${GREEN}  ✅ /workspace/otto-handler/secrets 디렉토리가 생성되었습니다${NC}"
  echo "  📌 GitHub App 비공개 키를 다음 위치에 저장하세요: /workspace/otto-handler/secrets/github-app-dev.pem"
else
  echo "  ℹ️  시크릿 디렉토리가 이미 존재합니다"
fi
...
echo ""
echo "   🔐 필수 설정:"
echo "   • COOKIE_SECRET: 쿠키 암호화 키"
echo "   • JWT_SECRET: JWT 토큰 서명 키"
echo ""
echo "   🐙 GitHub App 설정 (GitHub 연동 시 필요):"
echo "   • OTTO_GITHUB_APP_ID: GitHub App ID"
echo "   • OTTO_GITHUB_APP_NAME: GitHub App 이름"
echo "   • OTTO_GITHUB_WEBHOOK_SECRET: 웹훅 시크릿"
echo "   • OTTO_GITHUB_APP_PRIVATE_KEY: GitHub App 비공개 키"
echo ""
echo "   📋 GitHub App 생성 방법:"
echo "   1. https://github.com/settings/apps/new 접속"
echo "   2. 새 GitHub App 생성 후 App ID 복사"
echo "   3. Private key 생성 후 내용을 .env.dev에 붙여넣기"
echo ""
echo "📚 자주 사용하는 명령어:"
