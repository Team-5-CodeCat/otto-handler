...
export class GithubService {
    ...
    async registerApp(user: { userID: string }, dto: RegisterGithubAppDto) {
        try {
            this.logger.log(`GitHub 앱 등록 시작 - 사용자: ${user.userID}, 설치 ID: ${dto.installationId}`);
            ...
            this.logger.log(`GitHub 앱 등록 완료 - 사용자: ${user.userID}, 레포지토리 수: ${repositories.length}`);
            ...
            if (error.status === 401) {
                throw new UnauthorizedException('GitHub 액세스 토큰이 유효하지 않습니다.');
            }
    ...
    async handleWebhookEvent(event: any) {
        try {
            this.logger.log(`GitHub Webhook 이벤트 수신: ${event.action || 'unknown'}`);

            // GitHub App 설치 이벤트 처리
            if (event.action === 'created' && event.installation) {
                this.logger.log(`GitHub App 설치 완료 - 설치 ID: ${event.installation.id}`);
    ...
    async getBranches(user: { userID: string }, repo: string) {
        try {
            this.logger.log(`브랜치 목록 조회 시작 - 사용자: ${user.userID}, 레포: ${repo}`);
            ...
            if (!github?.accessToken) {
                throw new UnauthorizedException('GitHub 인증 정보가 없습니다. 먼저 GitHub 앱을 설치해주세요.');
            }
            ...
            if (!owner || !repoName) {
                throw new BadRequestException('잘못된 레포지토리 이름 형식입니다. (owner/repo 형식으로 입력해주세요)');
            }
            ...
            this.logger.log(`브랜치 목록 조회 완료 - 사용자: ${user.userID}, 레포: ${repo}, 브랜치 수: ${branchList.length}`);

            return branchList;

        } catch (error) {
            this.logger.error(`브랜치 목록 조회 실패 - 사용자: ${user.userID}, 레포: ${repo}`, error);
            ...
            if (error.status === 403) {
                throw new UnauthorizedException('해당 레포지토리에 접근 권한이 없습니다.');
            }
    ...
    async registerBranch(user: { userID: string }, repo: string, branch: string) {
        try {
            this.logger.log(`브랜치 등록 시작 - 사용자: ${user.userID}, 레포: ${repo}, 브랜치: ${branch}`);
            ...
            if (!project) {
                throw new BadRequestException('등록된 프로젝트가 없습니다. 먼저 프로젝트를 생성해주세요.');
            }
            ...
            this.logger.log(`브랜치 등록 완료 - 사용자: ${user.userID}, 프로젝트: ${project.projectID}, 레포: ${repo}, 브랜치: ${branch}`);
            ...
            this.logger.error(`브랜치 등록 실패 - 사용자: ${user.userID}, 레포: ${repo}, 브랜치: ${branch}`, error);
