...
export class LogStreamingGateway 
  implements ILogStreamingGateway, OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
   ...
   */
  private readonly clientSessions = new Map<string, {
  ...
  async handleConnection(client: Socket, authToken?: string): Promise<void> {
    try {
      ...
      const token = authToken || client.handshake.auth?.token || client.handshake.headers.authorization?.replace('Bearer ', '');
      
      if (token) {
        try {
          const payload = this.jwtService.verify(token);
          userId = payload.sub || payload.userId;
          this.logger.debug(`인증된 사용자 연결: ${userId} (socket: ${client.id})`);
        } catch (jwtError) {
          this.logger.warn(`JWT 인증 실패: ${jwtError.message}, 비인증 사용자로 처리`);
      ...
      this.logger.log(`클라이언트 세션 생성 완료: ${client.id} → ${session.sessionId}`);
      
    } catch (error) {
      this.logger.error(`클라이언트 연결 처리 실패: ${error.message}`, error.stack);
  ...
  async handleDisconnection(client: Socket): Promise<void> {
    try {
      ...
      if (clientSession) {
        ...
        // 🗑 세션 정리
        await this.logStreamingService.closeSession(clientSession.session.sessionId);
        this.clientSessions.delete(client.id);
        
        this.logger.log(`클라이언트 연결 종료 및 정리 완료: ${client.id}`);
      } else {
        this.logger.warn(`연결 종료된 클라이언트의 세션을 찾을 수 없음: ${client.id}`);
      }
      
    } catch (error) {
      this.logger.error(`클라이언트 연결 종료 처리 실패: ${error.message}`, error.stack);
    }
  }
  ...
  async handleSubscribeToLogs(
    ...
    try {
      ...
      this.logger.debug(`로그 구독 완료: client=${client.id}, room=${roomName}`);
  ...
  async handleUnsubscribeFromLogs(
    ...
    try {
      ...
      this.logger.log(`로그 구독 해제 요청: client=${client.id}, taskId=${taskId}`);
      ...
      this.logger.debug(`로그 구독 해제 완료: client=${client.id}, room=${roomName}`);
      
    } catch (error) {
      this.logger.error(`로그 구독 해제 처리 실패: ${error.message}`, error.stack);
      client.emit('unsubscription_error', {
  ...
  async handleUpdateLogFilter(
    ...
    try {
      ...
      this.logger.log(`로그 필터 업데이트 요청: client=${client.id}, taskId=${taskId}`);
      ...
      this.logger.debug(`로그 필터 업데이트 완료: client=${client.id}, taskId=${taskId}`);
      
    } catch (error) {
      this.logger.error(`로그 필터 업데이트 실패: ${error.message}`, error.stack);
      client.emit('filter_update_error', {
  ...
  async broadcastLog(taskId: string, logEntry: WorkerLogEntry): Promise<void> {
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        this.logger.debug(`로그 브로드캐스트: room=${roomName}, clients=${roomSize}, level=${logEntry.level}`);
      }
      
    } catch (error) {
      this.logger.error(`로그 브로드캐스트 실패: taskId=${taskId}, error=${error.message}`, error.stack);
    }
  }
  ...
  async broadcastPipelineProgress(
    ...
    try {
      ...
      // 📝 중요한 상태 변화는 별도 로깅
      if (['STAGE_COMPLETED', 'STAGE_FAILED', 'STAGE_CANCELLED'].includes(progress.status.toString())) {
        this.logger.log(`파이프라인 상태 변화 브로드캐스트: ${pipelineId} → ${progress.status} (Stage: ${progress.stageId})`);
      }
      
    } catch (error) {
      this.logger.error(`파이프라인 진행 상황 브로드캐스트 실패: pipelineId=${pipelineId}, error=${error.message}`, error.stack);
    }
  }
  ...
  async handleSubscribeToPipeline(
    ...
    try {
      ...
      this.logger.log(`파이프라인 구독 요청: client=${client.id}, pipelineId=${pipelineId}`);
      ...
      this.logger.error(`파이프라인 구독 처리 실패: ${error.message}`, error.stack);
      client.emit('subscription_error', {
  ...
  async handleGetServerStats(
    ...
    try {
      ...
      this.logger.error(`서버 통계 정보 조회 실패: ${error.message}`, error.stack);
