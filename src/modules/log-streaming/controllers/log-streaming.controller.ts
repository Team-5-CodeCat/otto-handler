...
export class LogStreamingController implements ILogStreamingController {
  ...
  streamLogs(
    ...
    this.logger.log(`SSE 로그 스트림 시작: taskId=${taskId}, level=${level}, workerId=${workerId}`);
    ...
    try {
      ...
      // 🔄 로그 스트림 시작 및 SSE 형식으로 변환
      const logStream$ = this.logStreamingService
        .startWorkerLogStream(taskId, filter)
        .pipe(
          // 🔄 WorkerLogEntry → SSEMessage 변환
          map((logEntry: WorkerLogEntry) => this.createSSEMessage('log', logEntry)),
          
          // ❌ 에러 처리: 클라이언트에게 에러 메시지 전송 후 스트림 종료
          catchError(error => {
            this.logger.error(`로그 스트림 에러: ${error.message}`, error.stack);
            const errorMessage = this.createSSEMessage('error', {
              message: '로그 스트림 에러가 발생했습니다',
      ...
      const subscription = logStream$.subscribe({
        ...
        next: (sseMessage: SSEMessage) => {
          ...
          if (process.env.NODE_ENV === 'development') {
            this.logger.debug(`SSE 메시지 전송: ${JSON.stringify(sseMessage).substring(0, 100)}...`);
          }
        },
        
        // ❌ 에러 발생 시 클라이언트에게 알림 후 연결 종료
        error: (error) => {
          this.logger.error(`SSE 스트림 에러: ${error.message}`);
          const errorMessage = this.createSSEMessage('error', {
            message: '스트림 연결 에러',
            timestamp: new Date().toISOString(),
          });
        ...
        // ✅ 정상적인 스트림 완료
        complete: () => {
          this.logger.log(`SSE 로그 스트림 완료: taskId=${taskId}`);
      ...
      response.raw.on('close', () => {
        this.logger.log(`클라이언트 연결 종료: taskId=${taskId}`);
      ...
      this.logger.error(`SSE 로그 스트림 초기화 실패: ${error.message}`, error.stack);
      throw new InternalServerErrorException('로그 스트림을 시작할 수 없습니다');
    }
  }
  ...
  streamPipelineProgress(
    ...
    this.logger.log(`SSE 파이프라인 진행 상황 스트림 시작: pipelineId=${pipelineId}`);
    ...
    try {
      ...
      const progressStream$ = this.logStreamingService
        ...
        .pipe(
          map((progress: PipelineProgress) => this.createSSEMessage('progress', {
      ...
      const subscription = progressStream$.subscribe({
        ...
        error: (error) => {
          this.logger.error(`파이프라인 SSE 스트림 에러: ${error.message}`);
          response.raw.end();
        },
        complete: () => {
          this.logger.log(`파이프라인 SSE 스트림 완료: pipelineId=${pipelineId}`);
          response.raw.end();
        }
      });
      ...
      // 🧹 클라이언트 연결 종료 시 정리
      response.raw.on('close', () => {
        this.logger.log(`파이프라인 클라이언트 연결 종료: pipelineId=${pipelineId}`);
        subscription.unsubscribe();
      });

    } catch (error) {
      this.logger.error(`파이프라인 SSE 스트림 초기화 실패: ${error.message}`, error.stack);
      throw new InternalServerErrorException('파이프라인 진행 상황 스트림을 시작할 수 없습니다');
    }
  }
  ...
  async getHealth(): Promise<{
    ...
    try {
      ...
      this.logger.error(`헬스체크 실패: ${error.message}`, error.stack);
